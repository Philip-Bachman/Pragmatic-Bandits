classdef ConfTopMOpt < handle
    % ConfTopMOpt works a MultiArmBandit of type 'bernoulli'
    ...
    methods
        function [self] = ConfTopMOpt(ma_bandit, top_m, a_0, b_0)
            % Constructor for multiarmed bandit ConfTopMOpt 
            if ~exist('a_0','var')
                a_0 = 2.0;
            end
            if ~exist('b_0','var')
        ...
        function [best_arm] = pick_arm(self, group, a_confs)
            % Pick an arm to pull from the managed MultiArmBandit.
            stats = self.bandit_stats;
            if ~exist('a_confs','var')    
                [gc a_confs] = self.compute_significance(group, stats, 1000);
            end
            a_unconfs = (1 - a_confs(group,:)) + 1e-5;
            best_arm = randsample(1:self.arm_count,1,true,a_unconfs);
            return
        end
        ...
        function [best_group] = run_trial(self, epsilon, g_confs, a_confs)
            ...
            if (rand() < epsilon)
                ...
                best_group = self.pick_group(g_confs);
                best_arm = self.pick_arm(best_group,a_confs);
            end
        ...
        function [results] = run_trials(self,...
            ...
            group_succ_probs = zeros(self.group_count, trial_rounds);
            init_pulls = self.uniform_allocation(init_rounds);
            [sig_groups g_confs a_confs] = self.get_significant_groups();
            ...
            for t_num=1:trial_rounds,
                ...
                if (t_num <= init_rounds)
                    ...
                    self.run_trial(epsilon, g_confs, a_confs);
                end
                % Compute the set of significant groups (sometimes)
                if (mod(t_num,10) == 0)
                    [sig_groups g_confs a_confs] = ...
                ...
                group_succ_probs(:,t_num) = succ_probs;
                ...
                if (mod(t_num, 50) == 0)
                    fprintf('%4.d: (%d, %.4f, %.4f)\n', t_num,...
                        numel(sig_groups),mean(g_confs),select_accs(t_num));
                end
            end
            results = struct();
            results.sig_counts = sig_counts;
            results.group_confs = group_confs;
            results.group_succ_probs = group_succ_probs;

classdef FreqTopMOpt < handle
    % FreqTopMOpt works a MultiArmBandit of type 'bernoulli'
    ...
    methods
        function [self] = FreqTopMOpt(ma_bandit, top_m, a_0, b_0)
            % Constructor for multiarmed bandit FreqTopMOpt 
            if ~exist('a_0','var')
                a_0 = 2.0;
            end
            if ~exist('b_0','var')
        ...
        function [best_arm] = pick_arm(self, group, arm_confs)
            ...
            returns = self.get_map_returns();
            a_returns = returns(group,:);
            ar_srt = sort(a_returns,'descend');
            gap_loc = (ar_srt(self.top_m) + ar_srt(self.top_m+1)) / 2;
            a_gaps = zeros(1,self.arm_count);
            a_post_vars = zeros(1,self.arm_count);
            for a=1:self.arm_count,
                a_pulls = self.bandit_stats(group,a).pulls;
                %[mu, sigma, muci, sigmaci] = normfit(a_pulls,0.9);
                %var_ucb = sigmaci(2)^2;
                mu = mean(a_pulls);
                var_ucb = var(a_pulls);
                a_post_vars(a) = var_ucb / numel(a_pulls);
                a_gaps(a) = abs(mu-gap_loc) / sqrt(a_post_vars(a));
            end
            [best_gap best_arm] = min(a_gaps);
            if (rand() < self.exp_rate)
                [best_var best_arm] = max(a_post_vars);
            end 
